...
const ManageOrders = () => {
    ...
    useEffect(() => {
        const url = `https://nameless-sea-82925.herokuapp.com/orders`;
        fetch(url)
            .then(res => res.json())
            .then(data => {
                setOrders(data)
    ...
    const handleApproveOrder = (id) => {
        ...
        const url = `https://nameless-sea-82925.herokuapp.com/approve/${id}`;
        fetch(url, {
            method: 'PUT',
            headers: {
    ...
    const handleDeleteOrder = (id) => {
        ...
        if (proceed) {
            const url = `https://nameless-sea-82925.herokuapp.com/deleteOrder/${id}`
            fetch(url, {
                method: 'DELETE'
            })
                .then(res => res.json())
